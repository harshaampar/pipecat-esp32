...
void pipecat_init_audio_capture() {
  // Initialize both microphone and speaker like working assistant (for WebRTC stability)
...
void pipecat_init_audio_encoder() {
  ESP_LOGI(LOG_TAG, "Initializing OPUS audio encoder...");
  ...
  if (opus_encoder_init(opus_encoder, SAMPLE_RATE, 1, OPUS_APPLICATION_VOIP) !=
      OPUS_OK) {
    ESP_LOGE(LOG_TAG, "Failed to initialize OPUS encoder");
    return;
  }
  ...
  if (!read_buffer || !encoder_output_buffer) {
    ESP_LOGE(LOG_TAG, "Failed to allocate audio buffers");
    return;
  }
  
  ESP_LOGI(LOG_TAG, "OPUS encoder initialized successfully - bitrate: %d, sample_rate: %d", 
           OPUS_ENCODER_BITRATE, SAMPLE_RATE);
}
