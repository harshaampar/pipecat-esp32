...
// Debug logging macro - separate from mbedtls debug to avoid linking issues
#ifndef CONFIG_PEER_CONNECTION_DEBUG
#define CONFIG_PEER_CONNECTION_DEBUG 1  // Enable by default for now
#endif

#if CONFIG_PEER_CONNECTION_DEBUG
#define DEBUG_LOG(fmt, ...) printf("[PEER_CONN] " fmt "\n", ##__VA_ARGS__)
#else
...
void peer_connection_set_remote_description(PeerConnection* pc, const char* sdp, SdpType type) {
  ...
  while ((line = strstr(start, "\r\n"))) {
    ...
    if (strstr(buf, "a=fingerprint")) {
      ...
      // Find the start of the actual fingerprint (after "a=fingerprint:sha-256 ")
      char* fingerprint_start = buf + 22;
      
      // Find the end of the fingerprint (either end of string or newline)
      char* fingerprint_end = fingerprint_start;
      while (*fingerprint_end != '\0' && *fingerprint_end != '\r' && *fingerprint_end != '\n') {
        fingerprint_end++;
      }
      
      // Calculate the actual fingerprint length
      size_t fingerprint_len = fingerprint_end - fingerprint_start;
      ...
      DEBUG_LOG("Found fingerprint #%d: %.*s", fingerprint_count, (int)fingerprint_len, fingerprint_start);
      
      // Use the FIRST fingerprint found (only set if not already set)
      if (fingerprint_count == 1) {
        strncpy(pc->dtls_srtp.remote_fingerprint, fingerprint_start, fingerprint_len);
        pc->dtls_srtp.remote_fingerprint[fingerprint_len] = '\0';
        DEBUG_LOG("Using fingerprint #1 as the active fingerprint");
      }
    }
  ...
  // Log final fingerprint selection summary
  DEBUG_LOG("SDP parsing complete. Found %d fingerprint(s). Using FIRST fingerprint: %s", 
            fingerprint_count, pc->dtls_srtp.remote_fingerprint);
